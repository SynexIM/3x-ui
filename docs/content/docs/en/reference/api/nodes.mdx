...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: This panel's own node-level fair-share policy, plus whether a control
        plane owns it (declarativelyManaged), in which case the panel refuses
        writes. Every rate is bit/s; 0 means the field is not enabled, never a
        default.
      url: '#this-panels-own-node-level-fair-share-policy-plus-whether-a-control-plane-owns-it-declarativelymanaged-in-which-case-the-panel-refuses-writes-every-rate-is-bits-0-means-the-field-is-not-enabled-never-a-default'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: This panel's own node-level fair-share policy, plus whether a control
          plane owns it (declarativelyManaged), in which case the panel refuses
          writes. Every rate is bit/s; 0 means the field is not enabled, never a
          default.
        id: this-panels-own-node-level-fair-share-policy-plus-whether-a-control-plane-owns-it-declarativelymanaged-in-which-case-the-panel-refuses-writes-every-rate-is-bits-0-means-the-field-is-not-enabled-never-a-default
