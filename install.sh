...
# there; a release that predates the checksum only warns, so installs still work.
verify_release_checksum() {
    ...
    sums=$(curl -fsSL --retry 3 --retry-delay 2 --connect-timeout 15 --max-time 60 \
    ...
    if [[ "${actual}" != "${expected}" ]]; then
        rm -f "${archive}"
        echo -e "${red}Checksum mismatch for ${asset}: the download does not match the published SHA256${plain}"
        exit 1
    fi
...
install_x-ui() {
    ...
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/${XUI_REPO}/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/${XUI_REPO}/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/${XUI_REPO}/main/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/${XUI_REPO}/main/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/${XUI_REPO}/main/x-ui.service.rhel"
                    ;;
            esac
